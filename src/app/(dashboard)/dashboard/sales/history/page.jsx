    ...
    return (
      <div className="w-full min-h-screen flex items-center justify-center bg-slate-50">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-slate-600 text-sm font-semibold animate-pulse">Loading orders history...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="w-full flex flex-col gap-6">
          ...
          <div className="flex flex-wrap gap-2 w-full md:w-auto">
            {['all', 'pending', 'confirmed', 'out_for_delivery', 'delivered', 'cancelled', 'returned'].map(status => (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-bold transition cursor-pointer border uppercase tracking-wide ${
                  statusFilter === status 
        ...
        {loading ? (
          <div className="w-full py-20 flex flex-col items-center justify-center gap-2">
            <BiLoaderAlt className="animate-spin text-4xl text-slate-800" />
            <p className="text-slate-500 text-sm font-semibold animate-pulse">Loading orders history...</p>
          </div>
        ) : filteredOrders.length === 0 ? (
          <div className="w-full bg-white border border-slate-200 py-16 px-6 text-center">
